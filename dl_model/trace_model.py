...
def main(args):
    ...
    train_dataset, val_dataset, test_gt = generate_data(input_config=config['input'], data_config=config['dataset'], is_train=(not args.test))
    ...
    if config['network']['loss_type'] == 'ce':  # classification task, determine output dimension based on number of labels
        ...
        config['network']['output_dims'] = 22#max([max([int(xx) for xx in x[0].split()]) for x in test_gt]) + 2
        print_and_log('Classification task detected, output dimension: %d' % config['network']['output_dims'])
    else:
...
def validate(val_loader, model, args, config, save_path=''):
    ...
    for i, (example_input, example_target) in enumerate(val_loader):
        ...
        print(example_input[0:1], example_input[0:1].shape)
