...
def main(args):
    ...
    train_dataset, val_dataset, test_gt = generate_data(input_config=config['input'], data_config=config['dataset'], is_train=(not args.test), training_file='both')
...
################################ save the file ###########################################
    ...
    if args.test:
        metric_test, loss_test, pred_all_test, fake_gts = validate(val_dataset, model, args, config, args.resume.split('/')[-2])
        if config['network']['loss_type'] == 'ctc' and config['input']['test_sliding_window']['applied']:
...
def validate(val_loader, model, args, config, save_path=''):
    ...
    for i, (example_input, example_target) in enumerate(val_loader):
        # if i < 1000: continue
        out = model(example_input[0:1])
        print('************ Input shape ************')
        print(example_input[0:1].shape)
        print('************ *********** ************')
        ...
        # print(out[0].shape)
        # break
        traced_script_module = torch.jit.trace(model, example_input[0:1])
        optimized_traced_model = optimize_for_mobile(traced_script_module)
        optimized_traced_model._save_for_lite_interpreter(save_path + '.ptl')
        out = optimized_traced_model(example_input[0:1])
        print('************ Traced model output ************')
        print(out)
        print('************ ******************* ************')
        print(f'\033[92mTraced successfully. **Please check that the original model output and traced model output are the same.**\033[0m')
        exit()
